...
from ._cell2fate_DynamicalModel_amortized_ModuleSpecificTime_module import \
Cell2fate_DynamicalModel_amortized_ModuleSpecificTime_module
...
class Cell2fate_DynamicalModel_amortized_ModuleSpecificTime(QuantileMixin, PyroSampleMixin, PyroSviTrainMixin, PltExportMixin, BaseModelClass):
    """
    ...
    **model_kwargs
    ...
    def __init__(
        ...
        **model_kwargs,
        ...
        if model_class is None:
            model_class = Cell2fate_DynamicalModel_amortized_ModuleSpecificTime_module
    ...
    def compute_module_summary_statistics(self, adata):
        ...
        torch.tensor(self.samples['post_sample_means']['A_mgON'][m,:])/torch.tensor(self.samples['post_sample_means']['beta_g']), axis = 1)
            ...
            adata.obs['Module ' + str(m) + ' State'
             ][self.samples['post_sample_means']['T_cm'][:,0,m] > self.samples['post_sample_means']['T_mON'][0,0,m]
              ] = 'Induction'
            adata.obs['Module ' + str(m) + ' State'
             ][self.samples['post_sample_means']['T_cm'][:,0,m] > self.samples['post_sample_means']['T_mOFF'][0,0,m]
              ] = 'Repression'
            adata.obs['Module ' + str(m) + ' State'
    ...
    def export_posterior(
        ...
        for m in range(self.module.model.n_modules):
            adata.obs['Time (hours) Module ' + str(m)] = self.samples['post_sample_means']['T_cm'][...,m].flatten() - np.min(self.samples['post_sample_means']['T_cm'][...,m].flatten())
            adata.obs['Time Uncertainty (sd) Module ' + str(m)] = self.samples['post_sample_stds']['T_cm'][...,m].flatten()
...
#         self.samples['post_sample_means']['mu_expression'][...,1]
    ...
    def export_posterior_quantiles(
        ...
        for quantile in quantiles:
            quantiles_dict[str(quantile)]=self.posterior_quantile(q=quantile,batch_size=batch_size, use_gpu=True)
