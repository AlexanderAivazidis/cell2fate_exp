...
class Cell2fate_DynamicalModel_TerminalStates_module(PyroModule):
    r"""
    ...
    def __init__(
        ...
        self.n_lineages = len(terminal_clusters)
        ...
        t_c_mean = torch.ones((self.n_obs,1,1))*0.5
        ...
        lineage_index = torch.zeros(self.n_obs, dtype = torch.long)
        for i in range(len(terminal_clusters)):
            lineage_index[clusters == terminal_clusters[i]] = i
        lineage_prior = torch.zeros(self.n_obs)
        lineage_prior[np.array([c in terminal_clusters for c in clusters])] = 1
        
        
        self.register_buffer(
            "lineage_weights_alpha",
            torch.tensor(lineage_weights_alpha*torch.ones(((1, 1, 1, self.n_lineages)))),
        )
        
        self.register_buffer(
            "lineage_prior",
            torch.tensor(lineage_prior, dtype = torch.float32),
        )           
        
        self.register_buffer(
            "lineage_index",
            torch.tensor(lineage_index, dtype = torch.long),
        ) 
        ...
        self.register_buffer("zero_k", torch.zeros(self.n_lineages, 1))
        ...
        self.register_buffer("zeros_k", torch.zeros(self.n_lineages, self.n_obs, self.n_vars))
    ...
    def create_plates(self, u_data, s_data, idx, batch_index):
        idx1 = idx[torch.where(self.lineage_prior[idx] == 0)[0]]
        idx2 = idx[torch.where(self.lineage_prior[idx] == 1)[0]]
        return pyro.plate("obs_plate", dim=-3, size = self.n_obs, subsample=idx), pyro.plate("obs_plate_1", dim=-3, size = self.n_obs, subsample=idx1), pyro.plate("obs_plate_2", dim=-3, size = self.n_obs, subsample=idx2)
    ...
    def forward(self, u_data, s_data, idx, batch_index):
        ...
        obs_plate, obs_plate_1, obs_plate_2 = self.create_plates(u_data, s_data, idx, batch_index)
        
        idx1 = torch.where(self.lineage_prior[idx] == 0)[0]
        idx2 = torch.where(self.lineage_prior[idx] == 1)[0]
        ...
        g_fg = pyro.sample( # (g_fg corresponds to module's spliced counts in steady state)
            ...
            .expand([self.n_lineages, self.n_modules, self.n_vars])
            .to_event(3)
        )
        A_kmgON = pyro.deterministic('A_kmgON', g_fg*gamma_g) # (transform from spliced counts to transcription rate)
...
#         t_mON = pyro.sample('t_mON', dist.Uniform(self.zero, self.one).expand([1, 1, self.n_modules]).to_event(2))
        t_delta = pyro.sample('t_delta', dist.Gamma(self.one*20, self.one * 20 *self.n_modules_torch).
                              expand([self.n_lineages, self.n_modules]).to_event(2))
        t_mON = torch.cumsum(torch.concat([self.zero_k.T, t_delta[:,:-1].T]), dim = 0).unsqueeze(0).unsqueeze(0)
        T_kmON = pyro.deterministic('T_kmON', T_max*t_mON.T.swapaxes(1,3))
        # Global switch off time for each gene:
        t_mOFF = pyro.sample('t_mOFF', dist.Exponential(self.n_modules_torch).expand([self.n_lineages, 1, 1, self.n_modules]).to_event(4))
        T_kmOFF = pyro.deterministic('T_kmOFF', T_kmON + T_max*t_mOFF)
        
        # =========== Mean expression according to RNAvelocity model ======================= #
        mu_total = torch.stack([self.zeros_k[:,idx,...], self.zeros_k[:,idx,...]], axis = -1)
        for k in range(self.n_lineages):
            for m in range(self.n_modules):
                mu_total[k,...] += mu_mRNA_continousAlpha_globalTime_twoStates(
                    A_kmgON[k,m,:], A_mgOFF, beta_g, gamma_g, lam_mi[m,...], T_c[:,:,0], T_kmON[k,:,:,m], T_kmOFF[k,:,:,m], self.zeros[idx,...])
        ...
        with obs_plate_1:
            ...
            pyro.sample("data_target", dist.GammaPoisson(concentration= stochastic_v_ag,
                       rate= stochastic_v_ag / mu[assignment[:,0,0],torch.arange(len(idx1)),:,:]).to_event(3),
                        obs=torch.stack([u_data[idx1,...], s_data[idx1,...]], axis = 2))
            
        with obs_plate_2:
            pyro.sample("data_target_terminal", dist.GammaPoisson(concentration= stochastic_v_ag,
                       rate= stochastic_v_ag / mu[self.lineage_index[idx2],idx2,:,:]).to_event(3),
                        obs=torch.stack([u_data[idx2,...], s_data[idx2,...]], axis = 2))