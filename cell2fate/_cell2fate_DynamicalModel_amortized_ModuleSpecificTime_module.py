...
class Cell2fate_DynamicalModel_amortized_ModuleSpecificTime_module(PyroModule):
    r"""
    ...
    def forward(self, x_data, idx, batch_index):
        ...
        with obs_plate:
            t_cm = pyro.sample('t_cm', dist.Normal(t_c_loc, t_c_scale).expand([batch_size, 1, self.n_modules]))
            T_cm = pyro.deterministic('T_cm', t_cm*T_max)
        # Global switch on time for each gene:
        t_mON = pyro.sample('t_mON', dist.Normal(t_c_loc-self.one/2, t_c_scale).expand([1, 1, self.n_modules]).to_event(2))
        T_mON = pyro.deterministic('T_mON', T_max*t_mON)
        # Global switch off time for each gene:
        t_mOFF = pyro.sample('t_mOFF', dist.Exponential(self.one * 2).expand([1, 1, self.n_modules]).to_event(2))
